...
假设你有 100 万个 1024 维的向量，用户输入一个查询向量，要找最相似的 Top-10。

```
暴力搜索（Flat）：
  计算查询向量与 100 万个向量的距离 → 排序 → 取 Top-10
  时间复杂度：O(N × D)，N=100万，D=1024
  耗时：约 500ms-2s（取决于硬件）
```
...
ANN 的核心权衡：**精度 vs 速度**。
- 精度 100% = 暴力搜索
- 精度 95-99% = ANN（速度快 10-1000 倍）
- 精度 80% = 更快的 ANN（速度更快但可能漏掉一些）
...
本项目的 Milvus 使用默认参数：`M=16`, `efConstruction=200`, `ef=64`。
...
本项目选择 HNSW 的原因：知识库在 10万-100万 向量级别，HNSW 在这个规模下是精度和速度的最优平衡。

## 六、在本项目中的体现

Milvus 创建 Collection 时默认使用 HNSW 索引。本项目的 `MilvusHybridStore` 通过 LangChain 封装，自动使用 HNSW。对于教学场景，默认参数已经足够，不需要手动调参。
