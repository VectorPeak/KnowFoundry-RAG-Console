...
**一个可迁移、可评测、可观测的通用 RAG 知识平台，帮助业务资料从入库走向可靠问答**
...
项目采用 `LangChain` 组织 RAG 主链路，使用 `Milvus 2.5.x` 承载 dense vector + BM25 sparse 混合检索，通过 `FastAPI / WebSocket` 提供接口与流式问答，并内置多场景切换、数据隔离、入库质量检查、RAG 回归评测和 LangSmith 观测能力。MCP（Model Context Protocol）、Agent、GraphRAG 和业务工作流编排属于二期扩展方向，当前一期重点验证企业级 RAG 从“能问答”到“可交付”的工程闭环。

项目的一大亮点是极易适配到你自己的业务中。得益于场景包、数据源、检索链路、Prompt Profile、评测集和治理脚本的可插拔设计，你可以快速将它结合到自己已有的项目里。无论你的背景是企业知识库、客服知识助手、制造业设备运维、合同/合规风控、保险理赔审核还是工程资料检索，都能找到适合自己的使用方式。具体使用策略会在后文 [业务场景](#2-业务场景) 与 [核心功能](#3-核心功能) 中展开。
