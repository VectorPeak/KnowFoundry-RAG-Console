...
如果是第一次学习或准备演示，建议先看 [教学最小路径](docs/TEACHING_MINIMAL_PATH.md)、[学习与演示总入口](docs/learning_demo_guide.md) 和 [标准演示 Runbook](docs/standard_demo_runbook.md)。前者用于授课减法，后两者按“怎么跑、怎么看代码、怎么演示”的顺序串起当前文档和主链路。
...
- 如何用 Milvus 2.6.x 内置 BM25 做 dense + sparse 混合检索；
...
| --- | --- | ---: | ---: | ---: | --- |
| `enterprise_knowledge` | HR、IT、财务制度 | 3 | 8 | 6 | 企业内部知识库智能问答平台 |
| `saas_support` | 账号、计费、开放集成 | 3 | 6 | 6 | SaaS 客服知识库智能助手 |
| `equipment_ops` | 巡检、告警、安全规范 | 3 | 6 | 6 | 制造业设备运维知识助手 |
| `compliance_qa` | 合同、审计、隐私保护 | 3 | 6 | 6 | 企业合规制度智能问答系统 |
| `cross_border_risk` | 海关、制裁、信用证、物流、单证 | 5 | 11 | 10 | 跨境贸易风控 RAG 知识问答平台 |
| `tender_contract_risk` | 招投标、合同、交付、验收、履约风险 | 5 | 11 | 10 | 招投标合规与合同履约 RAG 风控平台 |
| `insurance_claims` | 保单、理赔材料、责任、除外、赔付 | 5 | 10 | 10 | 保险理赔材料审核与 RAG 知识问答助手 |
| `engineering_project_qa` | 图纸、规范、进度、质量、安全资料 | 5 | 11 | 10 | 工程项目资料与施工规范 RAG 问答助手 |
...
| 向量库 | Milvus 2.6.x，兼容 2.5+ Hybrid Search |
| 稀疏检索 | Milvus `BM25BuiltInFunction` |
| 历史/反馈 | MySQL |
| 前端 | 原生静态页面 + 状态页 |
| RAG 回归验收 | 项目内置脚本 + JSON 报告 |
| 二期边界 | 一期源码不提前放 Agent 预留实现；二期用 LangGraph 新建 Agent 模块，并可选增加 A2A 协议适配 |
...
当前项目使用 Milvus 2.6.x 的内置 BM25 能力。dense、sparse、版本过滤和数据隔离统一放在 Milvus 中完成，避免维护本地 BM25、RedisSearch 或第二套索引。
...
连接别名注册。这样既保留 LangChain RAG 教学生态，又避免为了一个连接别名兼容问题
改成纯 PyMilvus 自研检索适配器。
...
LLM 接口，Key 欠费、无权限、模型名错误或服务地址不可用都会直接启动失败。正式授课和演示前
...
如果使用 `.env.compose.example`，继续启动 API 容器：

```powershell
docker compose up -d api
```

如果使用 `.env.local.example`，在宿主机启动 API：
...
脚本会自动读取 `.env` 中的 `ADMIN_API_TOKEN`，不需要把管理令牌写在命令行里。
...
课程里不再保留“一键发布检查”概念，避免学生在授课阶段把注意力放到脚本编排上。需要证明质量时，按上面的入库质量检查、RAG 回归验收、接口验收脚本分别演示即可。
...
本项目不再维护本地 Bad Case 导出、复核队列和提升脚本。课堂代码重点放在 RAG 主链路、source 推断、权限过滤、知识库版本、FAQ 命中、Prompt Profile、检索后处理和领域评测指标。
...
- 当前 `.env` 是“本机 API + localhost 端口”模式，还是“compose 网络主机名”模式；
- `docker-compose.yml` 和 `docker-compose.milvus.yml` 分别暴露了哪些宿主机端口；
- `docker ps` 里是否已经有相关容器存在，但没有把 MySQL / Milvus / API 端口映射到宿主机。

如果 `.env` 使用 `mysql`、`milvus` 和 `/app/models/...`，这代表 API 在 compose 网络中运行。
在 Windows 宿主机直接跑诊断时，脚本会把这些容器内视角的端口和路径降级为提示项，避免把
“运行模式不同”误判成项目不可用。

例如当前仓库里，标准 `docker-compose.yml` 默认把 MySQL 暴露到 `3306`，而 `docker-compose.milvus.yml`
把 MySQL 暴露到 `3307`。如果 `.env` 写的是 `MYSQL_PORT=3307`，但你实际启动的是标准
`docker-compose.yml` 那套服务，诊断报告会明确指出是 `.env` 与 compose 端口不一致，而不是只给出
一个抽象的“连接失败”。
...
| 文档 | 用途 |
| --- | --- |
| `docs/ENVIRONMENT_SETUP.md` | 本地和 Docker 启动说明 |
| `docs/PROJECT_ARCHITECTURE.md` | 当前架构、模块职责和验收命令 |
| `docs/current_architecture_flow.md` | 当前架构流程图 |
| `docs/phase1_quick_review.md` | 一期 RAG 快速复盘和交付检查 |
| `docs/evaluation_and_quality.md` | RAG 回归验收、入库质量检查和 LangSmith 观测说明 |
| `docs/multi_scenario_teaching_project.md` | 多场景教学设计 |
| `docs/phase1_quick_review.md` | 一期 RAG 快速复盘和交付检查 |
| `docs/` | 18 讲系统课程 + 8 个技术附录 |
| `docs/resume_project_pack.md` | 简历项目包装材料 |
| `docs/interview_faq.md` | 高频面试问答 |
| `docs/interview_playbook.md` | 面试讲解手册 |
| `docs/phase2_agent_boundary.md` | 二期 Agent 与一期 RAG 的能力边界 |
...
- `.env` 不提交真实 Key；
- `.env.example`、`.env.local.example`、`.env.compose.example` 只保留占位符；
- `logs/`、`reports/`、`.index_manifest/` 默认不作为线上公开材料；
